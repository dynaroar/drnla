...
[L363]  COND FALSE  !(y == 10 || x != 5)
        VAL         [x=5, y=0]
[L375]              if_too_big_3 = 1
        VAL         [if_too_big_3=1, x=5, y=0]
[L376]              reach_error()
        VAL         [if_too_big_3=1, x=5, y=0]
"""
...
c = 0
x0, y0 = Ints('x0 y0')
# mconstr = And(Not(And(x0 + y0 <= 19, -x0 - y0 <= -7)), 
#               Not(And(-y0 <= 10, x0 + y0 <= 19)),
#               Not(-x0 + y0 <= 1000))
# mconstr = True
mconstr = Not(-y0 <= -1000)
...
while c < 1000:
    ...
    if s.check() == sat:
        ...
        mconstr = And(mconstr, cconstr, Or([z3.Int(v.name()) != m[v] for v in m.decls()]))
        x = m[z3.Int('x0')]
        y = m[z3.Int('y0')]
